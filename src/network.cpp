...
static void do_ts_sockopt(int sock) {
...

  {
    int enable = SOF_TIMESTAMPING_RX_HARDWARE | SOF_TIMESTAMPING_RAW_HARDWARE |
                 SOF_TIMESTAMPING_SYS_HARDWARE | SOF_TIMESTAMPING_SOFTWARE | SOF_TIMESTAMPING_TX_HARDWARE | SOF_TIMESTAMPING_OPT_ID;
...
Network::Network(const std::string& franka_address,
  ...
  try {
    ...
    udp_socket_.setReceiveTimeout(Poco::Timespan{10000l * udp_timeout.count()});
